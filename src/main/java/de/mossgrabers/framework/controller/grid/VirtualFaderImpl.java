...
public class VirtualFaderImpl implements IVirtualFader {
    ...
    /**
     * Constructor. Does not update a slider on the grid. Use getColorState method
     * to draw the fader
     * yourself.
     *
     * @param host     The host
     ...
     */
    public VirtualFaderImpl(final IHost host, final IVirtualFaderCallback callback, final IPadGrid padGrid,
            final int index) {
        this.host = host;
    ...
    protected void moveFaderToDestination() {
        ...
        this.moveTargetValue = this.moveDirectionIsUpwards ? Math.min(current + this.moveDelay, this.moveDestination)
                : Math.max(current - this.moveDelay, this.moveDestination);
    ...
    /**
     * Special handling of pads for smoothing the fader, e.g. special handling of
     * 1st row.
     *
